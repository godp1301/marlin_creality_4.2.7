...
compile:
	docker run -it --rm -v $(shell pwd)/$(SCRATCH_DIR)/Marlin-$(MARLIN_RELEASE):/usr/src/Marlin-$(MARLIN_RELEASE) -w /usr/src/Marlin-$(MARLIN_RELEASE) platformio:build platformio run -e STM32F103RET6_creality

firmware: $(OUTPUT_DIR)/firmware.bin

${OUTPUT_DIR}/firmware.bin:
	cp $(SCRATCH_DIR)/Marlin-$(MARLIN_RELEASE)/.pio/build/STM32F103RET6_creality/firmware*.bin $(OUTPUT_DIR)/firmware.bin